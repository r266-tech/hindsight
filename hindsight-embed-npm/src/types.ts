...
/**
 * Options for {@link HindsightEmbedManager}.
 *
 * The manager is intentionally thin and pass-through: anything configurable
 * on the Python side (env vars or CLI flags) can be set here without needing
 ...
 * against `manager.getBaseUrl()`. This package does not ship its own HTTP
 * client.
 */
export interface HindsightEmbedManagerOptions {
  ...
  host?: string;
  /** Version of the `hindsight-embed` PyPI package to run via `uvx`. Default: `"latest"`. */
  embedVersion?: string;
   ...
   * new Python env var never requires a wrapper update.
   *
